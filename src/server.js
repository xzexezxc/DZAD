...
const HOST = '0.0.0.0';
const MAX_WORKER_QUEUE = Number(process.env.MAX_WORKER_QUEUE || 20);
...
const worker = spawn(process.execPath, [path.join(process.cwd(), 'src/worker.js')], { stdio: ['pipe', 'pipe', 'inherit'] });
let activeWorkerRun = null; let workerBuffer = '';
worker.stdout.on('data', chunk => { workerBuffer += chunk.toString(); for (const line of workerBuffer.split('\n').slice(0, -1)) handleWorkerMessage(line); workerBuffer = workerBuffer.split('\n').at(-1) || ''; });
worker.on('exit', code => { if (activeWorkerRun) finishRun(activeWorkerRun, { status: 'FAILED', errors: [{ category: 'worker', message: `Worker exited with code ${code}` }] }); });
function handleWorkerMessage(line) { try { const msg = JSON.parse(line); if (!activeWorkerRun || msg.id !== activeWorkerRun) return; if (msg.type === 'progress') updateProgress(msg.id, msg.progress); if (msg.type === 'result') finishRun(msg.id, msg.result); if (msg.type === 'error') finishRun(msg.id, { status: 'FAILED', errors: [msg.error] }); } catch {} }
...
function finishRun(id, result) { const entry = runs.get(id); if (entry) { entry.status = result.status; entry.result = result; } db.prepare('UPDATE scrape_runs SET status=?,result=?,updated_at=? WHERE id=?').run(result.status, JSON.stringify(result), now(), id); activeWorkerRun = null; pumpQueue(); }
function pumpQueue() { if (activeWorkerRun || !queue.length) return; const id = queue.shift(); const entry = runs.get(id); if (!entry) return pumpQueue(); activeWorkerRun = id; entry.status = 'RUNNING'; db.prepare('UPDATE scrape_runs SET status=?,updated_at=? WHERE id=?').run('RUNNING', now(), id); worker.stdin.write(`${JSON.stringify({ type: 'scrape', id, config: entry.config })}\n`); }

app.get('/api/health', (_req, res) => res.json({ ok: true, app: 'CATHPRN', version: '3.0.0', worker: worker.exitCode === null, queued: queue.length, active: Boolean(activeWorkerRun) }));
...
app.post('/api/ai-scraper/test', (req, res) => {
  const config = { ...(req.body || {}) }; if (!/^https?:\/\//i.test(config.galleryUrl || '')) return res.status(400).json({ error: 'galleryUrl must be an http(s) URL' });
  if (queue.length >= MAX_WORKER_QUEUE || activeWorkerRun) { /* queue is bounded; one dedicated worker protects API responsiveness */ }
  const id = crypto.randomUUID(); const createdAt = now(); const record = { id, status: 'QUEUED', config, result: null, createdAt };
  runs.set(id, record); db.prepare('INSERT INTO scrape_runs VALUES(?,?,?,?,?,?)').run(id, 'QUEUED', JSON.stringify(config), null, createdAt, createdAt); queue.push(id); pumpQueue(); res.status(202).json({ runId: id, status: 'QUEUED' });
});
...
app.post('/api/ai-scraper/runs/:id/pause', (req, res) => { if (activeWorkerRun !== req.params.id) return res.status(404).json({ error: 'Run is not active' }); worker.kill('SIGTERM'); activeWorkerRun = null; db.prepare('UPDATE scrape_runs SET status=?,updated_at=? WHERE id=?').run('PAUSED', now(), req.params.id); res.json({ paused: true }); });
app.post('/api/ai-scraper/runs/:id/resume', (req, res) => { const row = db.prepare('SELECT * FROM scrape_runs WHERE id=?').get(req.params.id); if (!row) return res.sendStatus(404); if (activeWorkerRun) return res.status(409).json({ error: 'Worker is busy' }); runs.set(row.id, { id: row.id, status: 'QUEUED', config: JSON.parse(row.config), result: null, createdAt: row.created_at }); queue.push(row.id); db.prepare('UPDATE scrape_runs SET status=?,updated_at=? WHERE id=?').run('QUEUED', now(), row.id); pumpQueue(); res.status(202).json({ resumed: true, runId: row.id, status: 'QUEUED' }); });
